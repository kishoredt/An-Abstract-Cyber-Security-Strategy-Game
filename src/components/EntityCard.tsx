...
type HighlightState = 'selected' | 'target' | null;
...
export function EntityCard({ entity, onClick, highlight, dim, acted }: EntityCardProps) {
  const tc: string = TEAM_COLORS[entity.team];
  ...
  const bgColor: string =
    ...
    entity.team === Team.UK  ? '#080f1a' : '#180808';

  const isGov: boolean =
    entity.id === 'uk_gov' || entity.id === 'ru_gov';
  ...
  return (
    ...
    >
      ...
      {entity.canBlackMarket && (
        <div style={{ color: '#aa77ff', fontSize: '9px', marginTop: '3px' }}>
          ★ Black Market
        </div>
      )}
    </div>
